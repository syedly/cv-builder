...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const template = req.nextUrl.searchParams.get('template') || 'modern';
  ...
  const buf = await generatePDF(gen.cvData as any, template);
  const pdfBytes = new Uint8Array(buf);

  const filename = `cv-${(gen.jobTitle || 'resume').toLowerCase().replace(/\s+/g, '-')}-${template}.pdf`;
